...
| EVID-005 | inspection | `src/lib.rs` finding and run-status types | Findings distinguish severity and determine run status. | Types implemented; dedicated tests pending. | partial |
...
| EVID-API-001 | fixture/review | WP-001 schema fixtures and WP-005 compatibility review | Versioned schema and compatibility policy exist. | Evidence collection deferred until WP-001 execution. | deferred |
| EVID-ADV-001 | test/review | WP-001 adversarial comparison and packet test matrix | Incomplete or misleading evidence is rejected or reported. | Evidence collection deferred until WP-001 execution. | deferred |
